...
class Test(unittest.TestCase):
    ...
    def test_a(self):
        ...
        print_fdt_default(
            {"table": tbl, "breaks": fd.breaks_info}, columns=[0, 1, 2], round=2, format_classes=False, row_names=True
        )
        print()

        print("Exemplo com seleção de colunas e formatação de classes")
        col = [0, 3, 5]
        print_fdt_default({"table": tbl, "breaks": fd.breaks_info}, columns=col, format_classes=True, pattern="{:.5e}")
